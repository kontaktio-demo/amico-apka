-- =====================================================================
--  AMICO - JEDNA KOMENDA (wklej calosc: Supabase -> SQL Editor -> Run)
--  Bezpieczne i idempotentne. NIE kasuje zadnych danych firmy.
--  Robi naraz: strukture + zabezpieczenia, proste logowanie (amico_wejscie),
--  magazyn "Dokumenty do pobrania" (Storage) oraz ustawia 3 hasla zespolu.
-- =====================================================================

create extension if not exists pgcrypto;

-- ---------- Tabele (tworzone tylko jesli nie istnieja) ----------
...
returns boolean language sql security definer stable set search_path = public as $$
  select exists (select 1 from public.amico_members m where m.workspace_id = w and m.user_id = auth.uid());
$$;
...
create policy amico_ws_select    on public.amico_workspaces for select to authenticated using (public.amico_is_member(id));
create policy amico_m_select     on public.amico_members    for select to authenticated using (public.amico_is_member(workspace_id));
create policy amico_state_select on public.amico_state      for select to authenticated using (public.amico_is_member(workspace_id));
...
-- ---------- Bootstrap (deterministyczny wybor firmy) ----------
...
begin
  ...
  select m.workspace_id into w from public.amico_members m
   ...
   order by (select count(*) from public.amico_members x where x.workspace_id = m.workspace_id) desc, m.created_at asc
   limit 1;
  if w is null then
    ...
    insert into public.amico_state (workspace_id, data, rev, updated_by) values (w, '{}'::jsonb, 0, auth.uid());
  else
  ...
  return query select m.workspace_id, m.rola, ws.join_code, ws.nazwa
    from public.amico_members m join public.amico_workspaces ws on ws.id = m.workspace_id
    where m.user_id = auth.uid() and m.workspace_id = w;
end; $$;
...
begin
  ...
  v_imie := public.amico_bezpieczny_tekst(p_imie, 120);
  if length(v_code) between 1 and 32 is not true then raise exception 'Nieprawidłowy kod dołączenia'; end if;
  select ws.id into w from public.amico_workspaces ws where ws.join_code = v_code;
  if w is null then raise exception 'Nieprawidłowy kod dołączenia'; end if;
  insert into public.amico_members (user_id, workspace_id, imie, email, rola)
...
-- ---------- Proste wejscie do jedynej firmy (dla logowania e-mail+haslo) ----------
...
-- ---------- Zapis stanu (CAS) + walidacja + limit 20MB ----------
...
begin
  if auth.uid() is null then raise exception 'Wymagane logowanie'; end if;
  if not public.amico_is_member(p_workspace) then raise exception 'Brak dostępu do tej firmy'; end if;
  if p_data is null or jsonb_typeof(p_data) <> 'object' then raise exception 'Nieprawidłowe dane stanu'; end if;
  if pg_column_size(p_data) > 20 * 1024 * 1024 then raise exception 'Dane firmy przekraczają dozwolony rozmiar'; end if;
  select s.rev into cur from public.amico_state s where s.workspace_id = p_workspace for update;
  if cur is null then
    insert into public.amico_state (workspace_id, data, rev, updated_by) values (p_workspace, p_data, 1, auth.uid());
    return query select true, 1::bigint, p_data;
  elsif cur = p_rev then
    update public.amico_state s set data = p_data, rev = s.rev + 1, updated_at = now(), updated_by = auth.uid()
     where s.workspace_id = p_workspace;
    return query select true, cur + 1, p_data;
  else
    return query select false, cur, (select s.data from public.amico_state s where s.workspace_id = p_workspace);
  end if;
end; $$;

-- ---------- Role + usuwanie czlonka + rotacja kodu ----------
...
begin
  if auth.uid() is null then raise exception 'Wymagane logowanie'; end if;
  if p_rola not in ('wlasciciel', 'kierownik', 'biuro', 'montazysta') then raise exception 'Nieprawidłowa rola'; end if;
  select m.rola into moja_rola from public.amico_members m where m.workspace_id = p_workspace and m.user_id = auth.uid();
  if moja_rola is null or moja_rola not in ('wlasciciel', 'kierownik') then raise exception 'Brak uprawnień'; end if;
  if p_user = auth.uid() then raise exception 'Nie można zmienić własnej roli'; end if;
  if p_rola = 'wlasciciel' and moja_rola <> 'wlasciciel' then raise exception 'Tylko właściciel może nadać rolę właściciela'; end if;
  if exists (select 1 from public.amico_members m where m.workspace_id = p_workspace and m.user_id = p_user and m.rola = 'wlasciciel')
     and p_rola <> 'wlasciciel'
     and (select count(*) from public.amico_members m where m.workspace_id = p_workspace and m.rola = 'wlasciciel') <= 1
  then raise exception 'Musi pozostać co najmniej jeden właściciel'; end if;
  update public.amico_members set rola = p_rola where user_id = p_user and workspace_id = p_workspace;
end; $$;
...
begin
  if auth.uid() is null then raise exception 'Wymagane logowanie'; end if;
  select m.rola into moja_rola from public.amico_members m where m.user_id = auth.uid() and m.workspace_id = p_workspace;
  if moja_rola is null or moja_rola not in ('wlasciciel', 'kierownik') then raise exception 'Brak uprawnień do usuwania osób z tej firmy'; end if;
  if p_user = auth.uid() then raise exception 'Nie można usunąć samego siebie'; end if;
  if exists (select 1 from public.amico_members m where m.workspace_id = p_workspace and m.user_id = p_user and m.rola = 'wlasciciel')
     and (select count(*) from public.amico_members m where m.workspace_id = p_workspace and m.rola = 'wlasciciel') <= 1
  then raise exception 'Nie można usunąć ostatniego właściciela'; end if;
  delete from public.amico_members m where m.user_id = p_user and m.workspace_id = p_workspace;
end; $$;
...
begin
  if auth.uid() is null then raise exception 'Wymagane logowanie'; end if;
  select m.rola into moja_rola from public.amico_members m where m.user_id = auth.uid() and m.workspace_id = p_workspace;
  if moja_rola is null or moja_rola not in ('wlasciciel', 'kierownik') then raise exception 'Brak uprawnień do zmiany kodu firmy'; end if;
...
do $$
begin
  if not exists (select 1 from pg_publication_tables where pubname = 'supabase_realtime' and schemaname = 'public' and tablename = 'amico_state')
  then alter publication supabase_realtime add table public.amico_state; end if;
exception when others then null;
end $$;

-- ---------- Magazyn "Dokumenty do pobrania" (Storage) ----------
insert into storage.buckets (id, name, public) values ('dokumenty', 'dokumenty', false)
...
-- ---------- Hasla zespolu (dla ISTNIEJACYCH kont) ----------
update auth.users set encrypted_password = extensions.crypt('Amico123!', extensions.gen_salt('bf', 10)),
  email_confirmed_at = coalesce(email_confirmed_at, now()), updated_at = now() where email = 'milena@amicco.pl';
update auth.users set encrypted_password = extensions.crypt('Amico123@', extensions.gen_salt('bf', 10)),
  email_confirmed_at = coalesce(email_confirmed_at, now()), updated_at = now() where email = 'biuro@amicco.pl';
update auth.users set encrypted_password = extensions.crypt('Amico123$', extensions.gen_salt('bf', 10)),
  email_confirmed_at = coalesce(email_confirmed_at, now()), updated_at = now() where email = 'wiktoria@amicco.pl';

-- ---------- Sprawdzenie kont ----------
select u.email, (u.encrypted_password is not null) as ma_haslo, m.rola as rola_w_firmie
from auth.users u left join public.amico_members m on m.user_id = u.id
where u.email in ('milena@amicco.pl','biuro@amicco.pl','wiktoria@amicco.pl') order by u.email;

-- Gotowe. Nic nie zostalo skasowane. (Pamietaj wylaczyc publiczna rejestracje:
